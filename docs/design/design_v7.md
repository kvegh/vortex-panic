# VORTEX PANIC — Design v7

*Updated 2026-07-26 — incorporates decisions v2–v7*
...
js/
  ...
  autopilot.js          — midpoint-based flight planner with correction zone
  ...
  flightlog.js          — timestamped event log with position tracking and progress milestones
...
4. Autopilot executes a planned flight profile (see below)
...
### Autopilot Logic — Midpoint Flight Profile

The autopilot plans a symmetric flight: accelerate for the first half of the distance, decelerate for the second half. Three phases:

1. **ACCELERATING**: full thrust toward target. Continues until the ship has traveled 50% of the total distance from its starting position.
2. **BRAKING**: full thrust against velocity (same g-level, symmetric deceleration). Continues until the ship enters the correction zone (last 0.1% of total distance).
3. **FINAL APPROACH**: stopping-distance logic fine-tunes arrival. Uses `(c²/a)(γ − 1)` to decide whether to brake or accelerate toward target. Handles numerical errors from discrete time stepping.
4. **ARRIVED**: distance < 1000km AND speed < 1 km/s → zero velocity, park ship, disengage.

On engage, the autopilot records the starting position and total distance. The midpoint is computed as 50% of total distance traveled, not as an absolute position — this works correctly regardless of flight direction.

The correction zone (0.1% of total distance) is small enough that the midpoint profile dominates the flight, but large enough to absorb substep errors at any time scale.

Autopilot runs inside the physics substep loop for accuracy at high time scales.
...
## Flight Log
- Version shown at startup: "VORTEX PANIC v7"
...
- Logged events:
  ...
  - Autopilot phase changes (ACCELERATING, BRAKING, FINAL APPROACH, ARRIVED)
  - **Progress milestones every 10%** of distance traveled — logs speed, gamma, G-force, fuel, ship time, Earth time, dilation, and distance to target
